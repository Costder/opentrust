...
**Architecture:** Two new top-level packages — `sdk/` (Python, pip-installable, includes MCP server as optional extra) and `sdk-ts/` (TypeScript, npm-publishable). Both wrap the live REST API at `https://api.opentrust.infiniterealms.io/api/v1`. Recommendation + risk logic is duplicated intentionally so each package has zero cross-language dependencies.
